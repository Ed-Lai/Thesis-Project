...
sgx_status_t ecall_call_aes(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* ecBytes, uint32_t ecBytesLen, uint8_t* userIDBytes, uint32_t userIDLength)
{
	sgx_status_t status;
	ms_ecall_call_aes_t ms;
	ms.ms_ecBytes = ecBytes;
	ms.ms_ecBytesLen = ecBytesLen;
	ms.ms_userIDBytes = userIDBytes;
	ms.ms_userIDLength = userIDLength;
	status = sgx_ecall(eid, 0, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t seal(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* data, uint32_t data_size, uint8_t* sealed_data, uint32_t sealed_size)
{
	sgx_status_t status;
	ms_seal_t ms;
	ms.ms_data = data;
...
sgx_status_t unseal(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* sealed_data, uint32_t sealed_size, uint8_t* unsealed_data, uint32_t unsealed_size)
{
	sgx_status_t status;
	ms_unseal_t ms;
	ms.ms_sealed_data = sealed_data;
	...
	ms.ms_unsealed_size = unsealed_size;
	status = sgx_ecall(eid, 2, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t ecall_calc_sealed_size(sgx_enclave_id_t eid, uint32_t* retval, uint32_t data_size)
{
	sgx_status_t status;
	ms_ecall_calc_sealed_size_t ms;
	ms.ms_data_size = data_size;
